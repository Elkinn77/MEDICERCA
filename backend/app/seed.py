...
def cargar() -> None:
    ...
    with ips_session(ips_1) as db_ips:
        ...
        for punto, medicamento, cantidad, fecha in [(p1, medicamentos["acetaminofen"], 50, None), (p2, medicamentos["acetaminofen"], 0, date.today() + timedelta(days=3)), (p1, medicamentos["losartan"], 0, date.today() + timedelta(days=5)), (p1, medicamentos["tramadol"], 10, None)]:
            obtener_o_crear(db_ips, Inventario, punto_id=punto.id, medicamento_id=medicamento.id, defaults={"cantidad": cantidad, "fecha_reabastecimiento": fecha})
        ...
        obtener_o_crear(db_ips, PrescripcionActiva, historia_id=historia.id, medicamento_id=medicamentos["losartan"].id, defaults={"fecha_formula": date.today() - timedelta(days=10), "vigente": True})
        db_ips.commit()
    with ips_session(ips_2) as db_ips:
        ...
        obtener_o_crear(db_ips, Inventario, punto_id=p3.id, medicamento_id=medicamentos["acetaminofen"].id, defaults={"cantidad": 20})
        obtener_o_crear(db_ips, Inventario, punto_id=p3.id, medicamento_id=medicamentos["losartan"].id, defaults={"cantidad": 15})
        obtener_o_crear(db_ips, Inventario, punto_id=p4.id, medicamento_id=medicamentos["acetaminofen"].id, defaults={"cantidad": 8})
        db_ips.commit()
    with ips_session(ips_3) as db_ips:
        ...
        obtener_o_crear(db_ips, Inventario, punto_id=p5.id, medicamento_id=medicamentos["acetaminofen"].id, defaults={"cantidad": 12})
        db_ips.commit()
